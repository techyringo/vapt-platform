...
class ReportAgent(BaseAgent):
    ...
    def __init__(self, scope: ScopeManager, config: AppConfig) -> None:
        ...
        self._executive_summary_cache: Optional[tuple[str, bool]] = None
    ...
    async def execute(self, task: AgentTask) -> list[Finding]:
        ...
        self._executive_summary_cache = None
        ...
        # ── Dual-model review of remaining Critical/High findings ──
        # Review only evidence-eligible findings, in bounded batches. This
        # avoids spending scarce API calls on observations the deterministic
        # quality gate will exclude anyway.
    ...
    async def _llm_fill_remediations(self, scan_result: ScanResult) -> None:
        ...
        if llm is None:
            return
        max_calls = max(0, int(os.environ.get("VAPT_LLM_REMEDIATION_LIMIT", "8")))
        calls = 0
    ...
    async def _llm_executive_summary(self, scan_result: ScanResult) -> tuple[str, bool]:
        ...
        """
        if self._executive_summary_cache is not None:
            return self._executive_summary_cache

        llm = self._get_llm()
        if llm is None:
            self._executive_summary_cache = (self._template_executive_summary(scan_result), False)
            return self._executive_summary_cache
        ...
        try:
            ...
            if text.startswith("```"):
                text = text.strip("`").strip()
            if text:
                self._executive_summary_cache = (text, True)
                return self._executive_summary_cache
            self._executive_summary_cache = (self._template_executive_summary(scan_result), False)
            return self._executive_summary_cache
        except Exception as exc:
