...
export interface CapabilityItem { name: string; displayName: string; state: 'ready' | 'on_demand' | 'blocked'; reason: string; actionLabel: string; }
...
export function OperationsWorkspace({ dockerReady, dockerInfo, totals, runtimeLogs, coverage, apiHealthy, onRefreshLogs, onDeleteLog, onRetry }: Props) {
  return <section>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0,1fr))', gap: 12, marginBottom: 16 }}>
      ...
      <MetricCard icon={Download} label="On demand / not eligible" value={`${totals.onDemand} / ${totals.blocked}`} sub="approved pull · credential · policy · adapter" tone="amber" />
    </div>
