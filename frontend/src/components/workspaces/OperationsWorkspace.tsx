...
export function OperationsWorkspace({ dockerReady, dockerInfo, totals, runtimeLogs, coverage, apiHealthy, onRefreshLogs, onDeleteLog, onRetry }: Props) {
  return <section>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0,1fr))', gap: 12, marginBottom: 16 }}>
      ...
      <MetricCard icon={Download} label="On demand / blocked" value={`${totals.onDemand} / ${totals.blocked}`} sub="approved pull · admin action" tone="amber" />
    </div>
    <div className="card-glass" style={{ padding: 14, marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 10 }}><div><h3 style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>Runtime Logs</h3><p style={{ marginTop: 2, fontSize: 11, color: 'var(--text-secondary)' }}>{runtimeLogs.length} file{runtimeLogs.length === 1 ? '' : 's'} available</p></div><button onClick={onRefreshLogs} className="btn btn-secondary"><RefreshCw size={14} aria-hidden="true" />Refresh</button></div>
